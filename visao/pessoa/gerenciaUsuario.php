        ...
        <div id="conteudo">
            ...
            <?
                ...
                if(($codigo != NULL && $codigo != "") || (($login != NULL && $login != "")  && ($senha != NULL && $senha != ""))){
                    include_once  "../../controle/pessoa/detalhesPessoa.php";
                }else{
                     $dados = NULL;
                }
                include_once  "../../controle/tipopessoa/procuraTiposPessoa.php";      
            ?>
            
            <form action="../../controle/pessoa/gerenciaPessoa.php" name="fgerenciaUsuario" method="post">
                <fieldset>
                    <input type="hidden" name="codigo" value="<?=$dados[codigo]?>"/>
                    <p>
                         <label>Tipo Pessoa:</label>
                         <select name="codtipopessoa">
                              <?
                                   while ($dadost = mysql_fetch_array($dados_tipo)) {
                                        if($dadost[codigo] == $dados[codtipopessoa]){
                                             echo("<option value='$dadost[codigo]' selected>$dadost[nome]</option>");
                                        }else{
                                             echo("<option value='$dadost[codigo]'>$dadost[nome]</option>");
                                        }
                              }?>
                         </select>
                    </p>
                    ...
                    <p>
                        <label>CPF:</label>
                        <input type="text" name="cpf" maxlength="11" size="15" value="<?=$dados[cpf]?>" onkeypress="mascaraInteiro(event);" placeholder="00000000000" onblur="valida_cpf(this);" title="Sem traços ou pontos">
                    </p>
                    
                    <p>
                        <label>Usuário:</label>
                        <input type="text" name="login" size="20" maxlength="20" value="<?=$dados[login]?>" required placeholder="Usuário para entrar no sistema"/>
                    </p>  

                    <p>
                        <label>Senha:</label>
                        <input type="password" name="senha" size="10" maxlength="10" value="<?=$dados[senha]?>" required placeholder="Senha para entrar no sistema"/>
                    </p>                     
                    
                    <p>
                        <label>Nascimento:</label>
                        <input type="date" name="nascimento" min="1920-01-01" value="<?=$dados[nascimento]?>" placeholder="00-00-0000"/>
                    </p>  
                    ...
                    <p>
                        <label>E-mail:</label>
                        <input type="email" name="email" size="50" maxlength="50" value="<?=$dados[email]?>" required placeholder="aaa@aaa.com.br"/>
                    </p>  
